...
| 대한민국학생발명전시회 | 📝 수동 | 발명교육포털(ip-edu.net)의 대통령상 전시관만 서버 렌더링됨. 우수상격 이하 전체 등급은 매년 공개되는 공식 "수상작품집" PDF 도록(연도별 다운로드 링크 확인함)에 실려있을 가능성이 높지만, 텍스트/스캔 여부 확인과 파서 구현이 아직 없어 `data/manual/student-invention.json`에 수동 보강 필요 |
| 도전! K-스타트업 | 📝 수동 | k-startup.go.kr / challengek.org에 연도별 전체 수상팀 아카이브가 없고 보도자료로만 공개됨. `data/manual/k-startup.json`에 수동 정리 |
| 창의적종합설계경진대회 | 📝 수동 | 수상작 게시판(ricee.or.kr)이 회원가입 후에만 열람 가능. 행사 사이트(e2festa.kr)에 매년 전체 출품작 PDF 도록이 공개되지만, 한글 본문이 CID 폰트라 텍스트 추출이 안 되고 OCR/비전 파서가 필요해(확인함) 아직 미자동화. `data/manual/capstone-design.json`에 수동 정리 |
