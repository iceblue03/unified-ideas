...
- 기본 검색(`app/api/search`)은 단어 단위 TF-IDF(BM25 스타일) 가중치 기반 유사도
  ...
  호출이 전혀 없어 완전히 무료입니다.
- "AI 검색" 옵션을 켰을 때만 OpenRouter의 무료 모델을 호출합니다([`app/api/search`](app/api/search/route.ts)
  의 `useAi` 분기, [`lib/openrouter.ts`](lib/openrouter.ts)). 환경변수 `openrouter_key`를 설정하지
  않으면 이 기능은 자동으로 꺼집니다. 이때 로컬 검색 상위 후보 데이터가 부족하면(원문이
  이미지/PDF뿐이라 summary가 짧은 경우) 화면에는 짧은 참고 문구 한 줄만 덧붙이고, 별도로 어떤
  데이터를 더 보강해야 하는지 모델에게 묻는 백그라운드 요청을 조용히 함께 보냅니다(`next/server`의
  `after()`로 응답을 막지 않음, 결과는 서버 로그에만 남음).
...
| 코드페어 (SW공모전·해커톤) | ⚠️ 반자동 | kcf.or.kr 공지사항 게시판에서 "수상작/수상팀/결과 발표" 키워드 공지를 찾아 본문 전체와 첨부 이미지·파일을 저장. **공식 사이트가 매 시즌 과거 아카이브를 통째로 지우는 것을 확인**했기 때문에(2026년 9월 기준, 예전 히스토리 페이지 전부 소실), 이 수집기가 실행될 때마다 발견한 공지를 저장소에 누적시켜 우리만의 히스토리를 만드는 방식입니다. 이 게시판은 결과를 팀명만 적힌 이미지 표로 올리는 경우가 흔해(2026 시즌 서면심사 결과에서 확인), 이미지·첨부파일은 attachments로만 보존하고 그 안 텍스트를 title/team으로 추측해 넣지 않습니다(팀명↔작품명 오인 방지). 지금 당장은 이번 시즌 최종 결과 발표 전이라 0건입니다. |
| 대한민국학생발명전시회 | ✅ 자동 | 발명교육포털(ip-edu.net)에서 그 해 "수상작품집" PDF를 찾아 다운로드 후, Playwright(Chromium) 안에서 pdf.js로 페이지를 렌더링해 [tesseract.js로 OCR](lib/ocr.ts)합니다. 앞부분 "수상작 목록" 표(이름/학교/학년/지역/출품명)와, 표가 가리키는 개별 작품 페이지(발명동기/발명내용/용도 및 효과)를 결합합니다. 표의 이름/학교 컬럼은 글자가 작아 OCR 오차가 있을 수 있습니다. |
