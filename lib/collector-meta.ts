...
export const KIPA_INVENTION_PATENT_META: CompetitionMeta = {
  ...
  method:
    ...
    "2006~2025년 각각 존재)를 순회하며 표(table.prize_table)를 HTML 파싱. " +
    "원래 수집 대상이던 '창의적종합설계경진대회'는 수상작 게시판(ricee.or.kr)이 robots.txt로 크롤링을 명시적으로 " +
    "막고 있어(Disallow: /, 해당 경로는 Allow 목록에도 없음) 자동 수집 대상에서 제외하고, 같은 개수를 유지하기 위해 " +
    "robots.txt상 크롤링이 허용되고 20개년치 데이터를 공개하는 이 대회로 대체했다.",
};
...
export const MAFRA_PUBLIC_DATA_STARTUP_META: CompetitionMeta = {
  ...
  method:
    ...
    "페이지네이션 순회하며 HTML 파싱. 원래 수집 대상이던 '정주영 창업경진대회'는 아산나눔재단이 선발팀 명단을 " +
    "구조화된 형태로 전혀 공개하지 않고(현재 사이트는 모집 안내 페이지뿐이고 역대 선발 기업 아카이브가 없음) " +
    "보도자료로만 흩어져 있어 자동 수집이 불가능해, 같은 개수를 유지하기 위해 구조화된 공개 아카이브가 있는 " +
    "이 대회로 대체했다.",
};
...
export const STUDENT_INVENTION_META: CompetitionMeta = {
  ...
  method:
    ...
    "각 항목의 상세(학교/수상자/발명동기 등)는 edit.do?idx=N GET으로 보강한다.",
};
